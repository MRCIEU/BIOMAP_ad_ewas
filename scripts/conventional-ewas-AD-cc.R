# ----------------------------------------
# ewaff EWAS script - 2 of 2 - with cell count adjustment
...
svs_file <- "ad-svs.tsv"
covar_file <- "covars-cc-svs.txt"
out_file <- "GS-AD-ewas-cc.Robj"
# output files - please replace [cohort_name] with your cohort name!
summary_stats_file <- "[cohort_name]_AD_ewas_summary_stats_cc.csv"
pheno_summary_table_outfile_1 <- "[cohort_name]AD-ewas-pheno_summary_numeric_vars_meanSD_cc.csv"
pheno_summary_table_outfile_2 <- "[cohort_name]AD-ewas-pheno_summary_numeric_vars_ttest_cc.csv"
pheno_summary_table_outfile_3 <- "[cohort_name]AD-ewas-pheno_summary_categorical_vars_counts_cc.csv"
ewas_report_file <- "[cohort_name]AD_ewas_report_cc.html"
...
run_ewas <- function(phen, pheno_dat, svs_file, meth_dat, IID, out_file, covs) 
{
  ...
  # Match meth to Pheno
  temp_meth <- meth_dat[, na.omit(match(as.character(temp_phen[[IID]]), as.character(colnames(meth_dat))))]
  temp_phen <- temp_phen[match(as.character(colnames(temp_meth)), as.character(temp_phen[[IID]])), ]
  ...
  obj <- meffil.ewas(temp_meth,variable=temp_phen[,phen],covariates=temp_phen[,covs], isva=F, sva=F, random.seed=23)
  print("ewas has run")
